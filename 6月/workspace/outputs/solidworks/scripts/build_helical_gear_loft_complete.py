...
def sketch_profile_on_selected_plane(model, rotation_rad):
    ...
    for k in range(z):
        ...
        next_minus_root = (k + 1) * pitch + rotation_rad - half_tooth - psi_p
...
def last_profile_name(model):
    ...
    if not profiles:
        raise RuntimeError("No profile sketch found after sketch creation.")
    return profiles[-1]


def create_offset_plane_from_front(model, offset_mm):
    front = select_named_plane(model, ["Front Plane", "前视基准面", "前基准面"])
    if front is None:
...
def create_loft_profiles(model):
    profile_names = []
    total_twist = math.radians(15.6770)
    stations = [0.0, 2.0, 4.0, 6.0, 8.0]

    select_named_plane(model, ["Front Plane", "前视基准面", "前基准面"])
    sketch_profile_on_selected_plane(model, 0.0)
    profile_names.append(last_profile_name(model))

    for z_mm in stations[1:]:
        plane = create_offset_plane_from_front(model, z_mm)
        if not plane.Select2(False, 0):
            raise RuntimeError(f"Could not select offset plane at {z_mm} mm.")
        rotation = total_twist * (z_mm / 8.0)
        sketch_profile_on_selected_plane(model, rotation)
        profile_names.append(last_profile_name(model))

    print(f"Created loft profiles: {profile_names}")
    return profile_names


def create_loft(model, profile_names):
    model.ClearSelection2(True)
    for index, name in enumerate(profile_names):
        feat = model.FeatureByName(name)
        if feat is None or not feat.Select2(index > 0, 1):
            raise RuntimeError(f"Failed to select loft profile: {name}")
    ...
    if feature is None:
        raise RuntimeError("Failed to create lofted helical gear body.")
    print(f"Created loft body: {feature.Name}")
...
def create_center_hole(model):
    ...
    feat = model.FeatureByName(hole_sketch)
    if feat is None or not feat.Select2(False, 0):
        raise RuntimeError(f"Failed to reselect bore sketch for FeatureCut3: {hole_sketch}")
...
def main():
    ...
    try:
        ...
        profiles = create_loft_profiles(model)
        model.ForceRebuild3(False)
        print_state(model, "After loft profile sketches")

        create_loft(model, profiles)
        model.ForceRebuild3(False)
        print_state(model, "After lofted helical body")
